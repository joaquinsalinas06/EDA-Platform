...
"dado un conjunto de puntos `(y_i, z_i)` y una consulta `(b2, b3)`, reportar
todos los puntos con `y_i ≤ b2` y `z_i ≤ b3`" — en tiempo `O(lg n)` y espacio
...
(hacia `z = +∞`), y la consulta `(b2, b3)` se convierte en un **rayo
horizontal** que sale de `(b2, b3)` hacia la izquierda. Un punto cumple
`y_i ≤ b2 ∧ z_i ≤ b3` si y solo si su rayo vertical es atravesado por el rayo
...
Los puntos cuyo rayo vertical queda a la izquierda de `b2` **y** por debajo
de la altura `b3` son exactamente los que el rayo horizontal atraviesa: los
dominados.
...
| rayo que se dispara | vertical, desde el punto de consulta | horizontal, desde `(b2, b3)` |
