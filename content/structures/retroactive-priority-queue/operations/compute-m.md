...
pide "el máximo entre `k` y todo lo que se eliminó en tiempo `≥ t`". Mirado
...
```
M = máx( {k} ∪ { k' ∉ Q_ahora : k' insertado en tiempo ≥ t' } )
```
...
`Q_ahora = {8}`, se inserta `Insert(2,5, 'insert(3)')` (tiempo `t=2,5`, entre
`t=2` y `t=3`). `M = máx({3} ∪ {2, 5}) = 5` (los eliminados en tiempo `≥ 2,5`
son `2` y `5`).
...
tiempo (`t'=0`, `Q_0=∅`). Los candidatos por la fórmula del puente son
`{k' insertado en t ≥ 0} \ Q_ahora = {5, 2, 8} \ {8} = {2, 5}` — el mismo
