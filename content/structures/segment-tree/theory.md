...
- **Build**: el árbol tiene exactamente 2n-1 nodos (n hojas, n-1 internos para
  un arreglo de tamaño n); construirlo visita cada nodo una vez, O(1) de
  trabajo por nodo → O(n) total.
- **Update**: sólo el camino raíz-hoja de la posición modificada cambia — a lo
  más ⌈lg n⌉ nodos, uno por nivel — y cada uno se recalcula en O(1) desde sus
  hijos → O(lg n).
- **Query**: un rango arbitrario se descompone en a lo más O(lg n) nodos
  ...
  (una por cada extremo del rango), el resto termina en O(1) → O(lg n) niveles
  × O(1) por nivel.
